...
export function Teams() {
  ...
  const filteredTeams = teams.filter(team => 
    ...
    (team.nome.toLowerCase().includes(searchTerm.toLowerCase()) || 
     (team.pais && team.pais.toLowerCase().includes(searchTerm.toLowerCase())))
  );

  // Group teams by country
  const groupedTeams = filteredTeams.reduce((acc, team) => {
    const country = team.pais || 'Sem País';
  ...
  return (
    <PageContainer>
      <PageHeader 
        title="Clubes" 
        description="Gerencie os clubes de futebol cadastrados no sistema."
        actions={
          isAdmin && (
            <button 
        ...
        <div className="p-4 border-b flex items-center justify-between">
          <div className="relative w-full max-w-sm">
            ...
            <input 
              ...
              placeholder="Buscar por clube ou país..."
                  ...
                  <div className="hidden md:block overflow-x-auto">
                    <table className="w-full text-sm text-left">
                      <thead className="text-xs uppercase text-muted-foreground border-b bg-background">
                        <tr>
                          <th className="px-6 py-3 font-medium">Clube</th>
                        ...
                        {groupedTeams[country].map((team) => (
                          <tr key={team.id} className="hover:bg-muted/30 transition-colors">
                            <td className="px-6 py-4 font-medium flex items-center gap-3">
                              ...
                              <Link to={`/teams/${team.id}`} className="text-primary hover:underline hover:opacity-80 transition-opacity">
                                {team.nome}
                              </Link>
                            </td>
      ...
      <Modal 
        ...
        title={selectedTeam ? 'Editar Clube' : 'Novo Clube'}
      >
        <TeamForm 
